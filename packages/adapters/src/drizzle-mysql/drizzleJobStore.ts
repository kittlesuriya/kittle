...
export interface DrizzleJobStoreConfig {
  jobsTable: AnyMySqlTable
  executionsTable: AnyMySqlTable
...
function isUniqueConstraintError(err: unknown): boolean {
  ...
  if (typeof e.message === "string" && e.message.includes("unique")) return true
  return false
}
...
function toStoredJob(row: Record<string, unknown>): StoredJob {
  return {
    ...
    nextAttemptAt: row.nextAttemptAt
      ? new Date(row.nextAttemptAt as string)
      : null,
    leaseOwner: (row.leaseOwner as string) ?? null,
    leaseExpiresAt: row.leaseExpiresAt
      ? new Date(row.leaseExpiresAt as string)
      : null,
...
export class DrizzleJobStore implements JobStore {
  ...
  private readonly jobsTable: AnyMySqlTable
  private readonly executionsTable: AnyMySqlTable
  ...
  async claimDue(args: ClaimDueJobsArgs): Promise<StoredJob[]> {
    ...
    const leaseExpiresAt = sql`NOW() + INTERVAL ${args.leaseDurationMs} MILLISECOND`
    const due = or(
      and(
      ...
      and(
        ...
        lt(this.jobs.attempts!, this.jobs.maxAttempts!),
        lt(this.jobs.leaseExpiresAt!, sql`NOW()`)
      )
    )
    ...
    const tokenReadback = claimedIds.map((id) =>
      and(
        eq(this.jobs.id!, id),
        eq(this.jobs.claimToken!, claimTokens.get(id)!)
      )
    )
  ...
  async renewLease(args: RenewLeaseArgs): Promise<boolean> {
    ...
    const leaseExpiresAt = sql`NOW() + INTERVAL ${durationMs} MILLISECOND`
    const result = await this.db
      .update(this.jobsTable)
      .set({ leaseExpiresAt })
      .where(
